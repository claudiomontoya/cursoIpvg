/**
 * seed.ts — Inicializa data.json con los 20 productos demo.
 *
 * Mismos productos que ../api/seed.ts y ../../python/mcp/api/seed.py.
 * Idempotente: si ya hay productos, los reemplaza.
 *
 * Correr:
 *     npm run api2:seed
 */

import { setProductos, countProductos, dataFilePath, type Producto } from './db.js';
...
const total = countProductos();
const categorias = [...new Set(PRODUCTOS.map((p) => p.categoria))].sort();
console.log(`✅ ${total} productos escritos en ${dataFilePath()}`);
