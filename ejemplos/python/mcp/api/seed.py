"""
seed.py — Pobla la collection `productos` con 20 productos demo.

Corre una sola vez. Si la collection ya tiene productos, los borra y vuelve a crear
(para que sea idempotente al iterar la demo).

Uso:
    python mcp/api/seed.py
"""

from db import productos_col, get_client
...
def main() -> None:
    try:
        ...
        print("❌ No puedo conectarme a MongoDB.")
        print("   Levantá la base con:  cd mcp/api && docker-compose up -d")
    ...
    print(f"✅ {col.count_documents({})} productos en la collection `productos`.")
    print("   Categorías:", sorted({p["categoria"] for p in PRODUCTOS}))
